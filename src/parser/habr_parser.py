...
def safe_request(url, retries=3, backoff_factor=1.5):
    for attempt in range(retries):
        try:
            ...
            logging.error(f"Ошибка (попытка {attempt + 1}): {e}")
...
def get_hub_urls():
    try:
        ...
        logging.error(f"Ошибка получения хабов: {e}")
        return {}

def get_articles_from_hub(hub_url, max_articles):
    articles = []
    page = 1
    
    while len(articles) < max_articles and page <= MAX_PAGES_PER_HUB:
        page_url = urljoin(hub_url, f'articles/page{page}/' if page > 1 else 'articles/')
...
def parse_article(url, category):
    try:
        ...
        body = soup.find('div', {'class': 'tm-article-body'})
...
def main():
    ...
    for category, hubs in categorized_hubs.items():
        ...
        for hub_url in hubs:
            ...
            try:
                articles = get_articles_from_hub(hub_url, min(needed - collected, 50))
